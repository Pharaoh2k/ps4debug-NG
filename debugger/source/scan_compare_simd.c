...
static size_t find_u32_avx1(const uint8_t *buf, size_t len, uint32_t target,
                            uint32_t *out, size_t max) {
    const uint32_t *p = (const uint32_t *)buf;
    size_t n = len / 4;
    size_t cnt = 0, i = 0;
    __m128i t = _mm_set1_epi32((int)target);
    for (; i + 8 <= n; i += 8) {
        __m256i v   = _mm256_loadu_si256((const __m256i *)(p + i));
        __m128i lo  = _mm256_castsi256_si128(v);
        __m128i hi  = _mm256_extractf128_si256(v, 1);
        __m128i eql = _mm_cmpeq_epi32(lo, t);
        __m128i eqh = _mm_cmpeq_epi32(hi, t);
        __m256i eq  = _mm256_insertf128_si256(_mm256_castsi128_si256(eql), eqh, 1);
...
size_t scan_simd_find_exact(unsigned char valtype, const uint8_t *buf, size_t len,
    ...
    if ((valtype == 4 || valtype == 5) && step == 4)
        return find_u32_avx1(buf, len, *(const uint32_t *)value, out_off, max_out);
